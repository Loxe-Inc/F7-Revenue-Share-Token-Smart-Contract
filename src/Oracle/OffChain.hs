...
updateOracle pkh = do
          ...
          let lookups     = Constraints.typedValidatorLookups (oracleScriptInst orcl)
              ...
              constraints = Constraints.mustPayToTheScript dat markerValue
                            <> Constraints.mustSpendScriptOutput (fst (head $ Map.toList markerUtxo)) (Redeemer $ toBuiltinData Update)
...
inspectOracle = do
            ...
            Contract.logInfo @String $ printf "Wallet own pkh is %s\n" $ show ownPkh
            let tn' = contractTokenName
                orcl = contractOracle ownPkh
            Contract.logInfo @String $ printf "Oracle is %s\n" $ show (orcl)
