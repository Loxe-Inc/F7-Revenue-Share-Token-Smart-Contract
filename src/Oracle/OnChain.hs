...
mkOracleValidator orcl dat r ctx =
  (traceIfFalse "Invalid signer" signedByWallet) &&
  (traceIfFalse "Marker token missing in outputs" outputHasMarker) &&
  case r of
      Update -> traceIfFalse "Marker token missing in inputs" $ inputHasMarker && outputHasMarker
      Use    -> traceIfFalse "Marker token missing in scr inputs" scriptInputHasToken
    where
      info :: TxInfo
      info = scriptContextTxInfo ctx

      signedByWallet :: Bool
      signedByWallet = txSignedBy info (unPaymentPubKeyHash $ wPkh orcl)
      ...
      inputHasMarker :: Bool
      inputHasMarker = assetClassValueOf (txOutValue ownInput) (oracleAsset orcl) == 1

      scriptInputHasToken :: Bool
      scriptInputHasToken =
            let markerIns = [i | i <- filter (isJust . toValidatorHash . txOutAddress) (txInInfoResolved <$> txInfoInputs info), assetClassValueOf (txOutValue i) (oracleAsset orcl) == 1]
            in case markerIns of
              [] -> False
              [_] -> True
